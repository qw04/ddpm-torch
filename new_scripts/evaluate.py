...
import traceback
from ddpm_torch.toy.toy_data import Gaussian1, Gaussian2, Gaussian25, Gaussian8, SwissRoll, TwoMoons
import numpy as np
...
def main():
    path_to_synth = "synth/images/train/experiment12/"
    DATASET = {
            "gaussian8": Gaussian8,
    ...
    temp = []
    for folder in tqdm(os.listdir(path_to_synth)):
        dataset = folder.split("_")[0]
        l_val = folder.split("_")[1]
        data = DATASET[dataset](1000)
        try:
            for file in tqdm(os.listdir(os.path.join(path_to_synth, folder))):
                
                iteration = file.split(".")[0]

                synth = np.loadtxt(os.path.join(path_to_synth, folder, file))[:1000]
            ...
            temp.append(f"error in dataset {dataset} and l value {l_val}")
            traceback.print_exc()

    np.savetxt("test1.txt", temp, fmt="%s")   
...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
    ...
    distance_block = DistanceBlockWithoutGPU(num_features)
...
# I only have access to 1 GPU so I don't really need this
# class DistanceBlock():
