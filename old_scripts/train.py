...
def train(synth, l, rank=0, args=None, temp_dir=""):
    ...
    chkpt_path = os.path.join(chkpt_dir, args.chkpt_name)
    ...
    if resume:
        try:
            ...
            trainer.load_checkpoint(args.chkpt_path, map_location=map_location)
            logger(f"Chekpoint loaded sccessfully from {args.chkpt_path}", flush=True)
    ...
    logger("Training completed!", flush=True)
    ...
    def sample_fn(n, diffusion, model, shape_, device):
        ...
        shape = (n, ) + shape_
        print("shape of samples is", shape, flush=True)
        sample = diffusion.p_sample(denoise_fn=model, shape=shape, device=device, seed = randint(0, 1000))
        synth = sample.cpu().numpy()
        return synth
        
    return partial(sample_fn, diffusion=diffusion, model=model, shape_ = trainer.shape, device=train_device)



@errors.record
def main(synth, l, **kwargs):
    from argparse import ArgumentParser

    parser = ArgumentParser()
    parser.add_argument("--config-path", type=str, help="path to the configuration file")
    parser.add_argument("--exp-name", type=str, help="name of the current experiment run")
    parser.add_argument("--dataset", choices=DATASET_DICT.keys(), default="mnist")
    ...
    parser.add_argument("--resume", default=True, action="store_true", help="to resume training from a checkpoint")
    ...
    parser.add_argument("--distributed", default=False, action="store_true", help="whether to use distributed training")
    parser.add_argument("--rigid-launch", default=False, action="store_true", help="whether to use torch multiprocessing spawn")
    ...
    if args.distributed and args.rigid_launch:
        ...
        **5. supports multi-node training
        """
        synth = train(args=args, synth=synth, l=l)
        return synth
